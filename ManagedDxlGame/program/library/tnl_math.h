...
namespace tnl {
	...
	/*
	* 
	* EasyAdjustObjectVelocity 
	* 
	tnl::Vector3 pos = { 500, 300, 0 };
	tnl::Vector3 move_vel;
	tnl::Vector3 past_move_vel;
	...
	void gameMain(float delta_time) {
		...
		if (tnl::Input::IsKeyDown(eKeys::KB_LEFT)) rot -= tnl::ToRadian(1);
		if (tnl::Input::IsKeyDown(eKeys::KB_RIGHT)) rot += tnl::ToRadian(1);

		if (tnl::Input::IsKeyDown(eKeys::KB_UP)) {
			tnl::Quaternion qtn = tnl::Quaternion::RotationAxis({ 0, 0, 1 }, rot);
			move_vel += tnl::Vector3::TransformCoord({ 0, -0.5f, 0 }, qtn) ;
		}
		if (tnl::Input::IsKeyDown(eKeys::KB_DOWN)) {
			tnl::Quaternion qtn = tnl::Quaternion::RotationAxis({ 0, 0, 1 }, rot);
			move_vel -= tnl::Vector3::TransformCoord({ 0, -0.1f, 0 }, qtn);
		}

		pos += move_vel;

		tnl::EasyAdjustObjectVelocity(20, 98, 0.65f, past_move_vel, move_vel, center_of_gravity);

		DrawRotaBox(pos, 30, 50, rot);
		DrawCircle(pos.x + center_of_gravity.x, pos.y + center_of_gravity.y, 10, -1, false);
	}
	*/
